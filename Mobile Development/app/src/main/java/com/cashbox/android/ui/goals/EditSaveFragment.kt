...
class EditSaveFragment : Fragment(R.layout.fragment_edit_save) {
    ...
    private fun setupEditText() {
        binding.edtDescription.setText(DataHelper.saveDescription)
        binding.edtAmount.setText(DataHelper.saveAmount.toIndonesianNumberString())
        binding.edtDate.setText(convertDateToIndonesianFormat(DataHelper.saveDate.substring(0, 10)))
    ...
    private fun setupDataStoreUpdate(description: String, amount: Long, date: String) {
        viewLifecycleOwner.lifecycleScope.launch {
            combine(userPreference.userToken, userPreference.userUid) { token, uid ->
                Pair(token, uid)
            }.collect { (token, uid) ->
                setupViewModel(token)
                addSaveViewModel.editSave(
                    DataHelper.saveId,
                    SaveBody(uid, DataHelper.goalsId, description, amount, date)
                )
                setupObservers()
            }
        }
